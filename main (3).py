...
def add_student():
            ...
            if ret==0:
                ...
                cur.execute("INSERT into STUDENTS values(?,?,?,?,?,?)",(student_name.get(),int(id_number.get()),year_level.get(),(gender.get()),course_code.get(),address.get()))
                conn.commit()
                conn.close()
...
if __name__=="__main__":
    ...
    e7=Entry(root,textvariable=course_code)
    e7.place(x=100,y=240)
    e8=Entry(root,textvariable=course_name)
    ...
    t1=Text(root,width=60,height=20, fg="yellow", bg="black")
...
#    b8=Button(root,text="ADD COURSE NAME/COURSE CODE",command=add_course,width=30, fg="yellow", bg="black")
